...
export async function POST(request: Request, { params }: Ctx) {
  ...
  const change = await prisma.bargeChangeLog.create({
    data: { shipmentId: id, ...parsed.data, changedById: session.user.id, status: "active" },
  });

  // Update shipment bargeName to new value
  await prisma.shipment.update({
    where: { id },
    data: { bargeName: parsed.data.newBarge },
  });
