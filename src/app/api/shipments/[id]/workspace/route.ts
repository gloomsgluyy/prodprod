...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const buyerSummary = { buyerQtyPlan: Number(shipment.qtyPlan ?? 0), blQty: Number(shipment.blQty ?? shipment.qtyFinal ?? shipment.qtyLoaded ?? 0), dischargedQty: shipment.dischargedQty == null ? null : Number(shipment.dischargedQty), paymentStatus: serialPayments[0]?.status ?? "pending", documentAlert: pendingDocuments, overallStatus: shipment.status };
  const quantityResult = { planQty: Number(shipment.qtyPlan ?? 0), blQty: Number(shipment.blQty ?? shipment.qtyFinal ?? shipment.qtyLoaded ?? 0), dischargedQty: shipment.dischargedQty == null ? null : Number(shipment.dischargedQty), factoryQty: shipment.factoryQty == null ? null : Number(shipment.factoryQty), lossGain: shipment.factoryQty != null && shipment.blQty != null ? Number(shipment.factoryQty) - Number(shipment.blQty) : null };
  return NextResponse.json({ data: { shipment, children, issues, documents, payments: serialPayments, quality, communication, progress, buyerSide: { summary: buyerSummary, quantityResult }, summary: { buyerQtyPlan: Number(shipment.qtyPlan ?? 0), allocatedQty, actualLoadedQty, childLoadedQty, childFinalQty: childQty("finalQty"), remainingQty: Number(shipment.qtyPlan ?? 0) - allocatedQty, bargeCompleted: children.filter((child) => ["completed", "loaded"].includes(child.status)).length, bargeTotal: children.length, openIssueCount: issues.length, criticalIssueCount, pendingDocuments, overduePayments, qualityStatus: quality?.status ?? null, overallStatus: criticalIssueCount ? "critical" : issues.length ? "at_risk" : shipment.status === "completed" ? "complete" : "in_progress" } } });
}
