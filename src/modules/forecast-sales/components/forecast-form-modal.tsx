...
export function ForecastFormModal() {
  ...
  useEffect(() => {
    if (!watch("forecastMonth") && customFields.offerDate) setValue("forecastMonth", customFields.offerDate.slice(0, 7));
  }, [customFields.offerDate, setValue, watch]);
          ...
          <form ref={formRef} onSubmit={handleSubmit(onSubmit)} noValidate className="flex flex-col gap-5">
            ...
            <div className="space-y-5">
              ...
              <FormSection number="5" title="Base Price">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3"><div className="field"><label className="field__label text-xs">Base Price Method</label><select className="select" value={customFields.basePriceMethod} onChange={(e) => setCustomField("basePriceMethod", e.target.value)}><option value="formula">Formula / Wording</option><option value="calculator">From Calculator History</option><option value="fixed">Fixed Final Price</option></select></div><div className="field"><label className="field__label text-xs">Price Basis *</label><select className="select" {...register("priceBasis")}><option value="">Select price basis</option><option value="ICI 1">ICI 1</option><option value="ICI 2">ICI 2</option><option value="ICI 3">ICI 3</option><option value="ICI 4">ICI 4</option><option value="ICI 5">ICI 5</option><option value="HBA">HBA</option><option value="Fixed">Fixed</option></select>{errors.priceBasis && <p className="text-xs text-danger mt-0.5" role="alert">{errors.priceBasis.message}</p>}</div><div className="field"><label className="field__label text-xs">Formula / Reference</label><input className="input" value={customFields.formula} readOnly={customFields.basePriceMethod === "calculator"} onChange={(e) => setCustomField("formula", e.target.value)} placeholder="ICI 3 + adjustment" /></div><div className="field"><label className="field__label text-xs">Average Period</label><select className="select" value={customFields.averagePeriod} onChange={(e) => setCustomField("averagePeriod", e.target.value)}><option value="latest">Latest</option><option value="1w">1 Week</option><option value="2w">2 Weeks</option></select></div><F id="salesPriceEst" label="Target Selling Price (USD/MT)" type="number" /></div>
              ...
              <FormSection number="8" title="Payment & Independent Surveyors"><div className="grid grid-cols-1 sm:grid-cols-2 gap-3"><F id="paymentTerm" label="Payment Term *" placeholder="LC at Sight / TT" /><F id="surveyor" label="Surveyor Selection *" placeholder="SGS / Intertek" /></div></FormSection>
